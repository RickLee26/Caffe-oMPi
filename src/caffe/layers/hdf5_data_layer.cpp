...
void HDF5DataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // Reshape blobs.
  int batch_size = this->layer_param_.hdf5_data_param().batch_size();
  if(this->layer_param_.phase() != TEST){
    batch_size = Caffe::divide_batch(batch_size);
  }
...
bool HDF5DataLayer<Dtype>::Skip() {
  ...
  */
  // for all solvers will attend testing, in order to balance delay
  bool keep = (offset_ % size) == rank; 
  return !keep;
}
