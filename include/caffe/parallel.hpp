...
template<typename Dtype>
class ThreadLocal : public Params<Dtype>,
                    public Solver<Dtype>::Callback,
                    public Net<Dtype>::Callback {

  public:
    ThreadLocal(shared_ptr<Solver<Dtype> > solver);
    vector<ThreadLocal<Dtype>* > *locals_;

    void Run();
    void syncData();      // get data from root_rank
    void syncNodeData();  // for root_rank

  protected:
    void on_start() {}
    void run(int layer);
    void on_gradients_ready();
    void on_test(Dtype &loss);
